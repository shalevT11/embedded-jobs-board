...
5. איכות קישורים: **`link` חייב להצביע על דף המשרה הספציפית — לעולם לא על דף חיפוש או דף קריירה כללי.** סדר עדיפות לקישור: ‏(1) **לינקדאין** — `il.linkedin.com/jobs/view/<slug-id>` — דפים ציבוריים שנפתחים בלי חשבון, וההגשה דרך לינקדאין מגיעה ישירות ל-ATS של החברה (שלו מקבל שם לפחות תשובות/דחיות, בניגוד לדרושים); ‏(2) דף קריירה רשמי של החברה; ‏(3) דרושים — פורמט `https://www.drushim.co.il/job/<ID>/<refcode>/`; ‏(4) ‏AllJobs — פורמט `https://www.alljobs.co.il/Search/UploadSingle.aspx?JobID=<id>` (חיפוש: `SearchResultsGuest.aspx?freetxt=...`). אגרגטורים (talentify/builtin/bandana/secrethunter) — להימנע, מתיישנים מהר. הקישורים העמוקים הישנים של jobs.intel.com מתים — פורטל intel.wd1.myworkdayjobs.com. אם אין שום קישור ישיר, מותר דף חיפוש רק עם note שמסביר מה לחפש.
